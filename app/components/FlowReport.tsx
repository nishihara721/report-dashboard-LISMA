...
function FlowTableWrapper({
  ...
  useEffect(() => {
    ...
    fetch(`/api/report-by-flow?flow=${encodeURIComponent(flowValue)}&${apiDateRange}`)
      .then((res) => res.json())
      .then((data) => { setRows(Array.isArray(data) ? data : []); setLoading(false); })
      .catch(() => setLoading(false));
  }, [flowValue, apiDateRange]);
  ...
  return (
    <div className="min-w-[600px]">
      <h3 className="text-sm font-semibold text-[#3A5A6A] mb-2 px-1">{flowValue}</h3>
...
export default function FlowReport() {
  const [flowValues, setFlowValues] = useState<string[]>([]);
  ...
  function toggleFlow(flow: string) {
    setSelectedFlow((prev) =>
      prev.includes(flow) ? prev.filter((v) => v !== flow) : [...prev, flow]
    );
  }
  ...
  return (
    <div>
      ...
      {selectedFlow.length === 0 ? (
        <p className="text-sm text-[#5A7A8A]">フローを選択してください</p>
      ) : (
        <div className="flex gap-6 overflow-x-auto pb-4">
          {selectedFlow.map((flow) => (
            <FlowTableWrapper
              key={flow}
              flowValue={flow}
