...
export default function SummaryTable() {
  ...
  function renderTable(rows: SummaryRow[], title: string) {
          ...
          <table className="min-w-full border-collapse text-sm text-[#3A5A6A]">
            <thead>
              <tr className="bg-[#7BB8D4]">
                {['', 'CL数', '友だち追加数', '友だち追加率', 'CV数', 'CVR', 'CPF', 'CPA', '広告費'].map((h) => (
              ...
              <tr className="bg-[#D6E8F2] font-semibold" style={{ boxShadow: '0 2px 0 #C8DCE8' }}>
                ...
                <td className="px-4 py-2 text-right bg-[#D6E8F2]">{total.adCost > 0 ? `¥${total.adCost.toLocaleString()}` : '-'}</td>
              </tr>
            </thead>
            <tbody>
              {rows.map((row) => (
                <tr key={row.label} className="border-b border-[#EEF3F6] hover:bg-[#F5F8FA]">
                  ...
                  <td className="px-4 py-2 text-right">{row.adCost > 0 ? `¥${row.adCost.toLocaleString()}` : '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <div>
      {renderTable(data.byMonth ?? [], '月別レポート')}
      {renderTable(data.byFlow ?? [], 'フロー別レポート')}
      {renderTable(data.byMedia ?? [], 'メディア別レポート')}
    </div>
  );
}