...
function CodeTableWrapper({
  ...
  useEffect(() => {
    ...
    fetch(`/api/report-by-code?code=${encodeURIComponent(codeValue)}&${apiDateRange}`)
      .then((res) => res.json())
      .then((data) => { setRows(Array.isArray(data) ? data : []); setLoading(false); })
      .catch(() => setLoading(false));
  }, [codeValue, apiDateRange]);
  ...
  return (
    <div className="min-w-[600px]">
      <h3 className="text-sm font-semibold text-[#3A5A6A] mb-2 px-1">{displayName}</h3>
...
export default function CodeReport() {
  ...
  return (
    <div>
      ...
      <SelectorBar
        label="コード選択："
        values={codeValues.map(codeLabel)}
        selected={selectedCode.map(codeLabel)}
        onToggle={(label) => {
          const code = codeValues.find((c) => codeLabel(c) === label) ?? label;
          toggleCode(code);
        }}
        emptyMessage="読み込み中..."
      ...
      {selectedCode.length === 0 ? (
        <p className="text-sm text-[#5A7A8A]">コードを選択してください</p>
      ) : (
